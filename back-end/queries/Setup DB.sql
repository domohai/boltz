...
CREATE TABLE `parcel` (
  ...
  `type` enum('docs', 'package') DEFAULT 'package',
  ...
  `curr_point` enum('src_service_p','src_collection_p','des_collection_p','des_service_p') DEFAULT 'src_service_p',
  `moving_to` enum('src_collection_p','des_collection_p','des_service_p') DEFAULT NULL,
  `status` enum('Chờ xử lý','Đang vận chuyển','Chờ trả hàng','Đã trả hàng','Đã huỷ') DEFAULT 'Chờ xử lý',
  `cost` INT NOT NULL,
