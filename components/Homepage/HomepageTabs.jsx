...
import TrackingParcel from "@components/Homepage/TrackingParcel";
...
const HomepageTabs = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-start h-fit my-[6rem]">
      <Tabs
        variant="underlined"
        aria-label="Tabs"
      ...
      <div className="mt-4 w-full md:w-3/4">
        {activeKey === 'tracking' ? <TrackingParcel/> : null}
        {activeKey === 'estimation' ? estimateParcel : null}
        {activeKey === 'restricted' ? restrictedParcel : null}
      </div>
    </div>
  );
};
