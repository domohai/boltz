...
const ConfirmSP = () => {
  ...
  const fetchParcels = async () => {
    ...
    try {
      const res = await fetch(`/api/service_staff/confirm_parcel?service_point_id=${String(service_point_id)}`, {
        method: "GET",
      });
  ...
  const updateParcels = async (e) => {
    ...
    try {
      const res = await fetch(`/api/service_staff/confirm_parcel`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ parcel_ids: parcelArray, status: "Chờ trả hàng" }),
      });
      const data = await res.json();
      if (data.ok) {
