...
const CollectionPointPage = () => {
  ...
  const fetchParcels = async (start_date, end_date) => {
    const waitForCollectionPointId = async () => {
    ...
    try {
      // Ensure collection_point_id is available before proceeding
      await waitForCollectionPointId();

      // Proceed with fetching parcels
      const res = await fetch(
        `/api/collection_manager?collection_point_id=${collection_point_id}&start_date=${start_date}&end_date=${end_date}`,
        {
          method: 'GET',
          headers: {
